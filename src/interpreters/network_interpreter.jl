function interpret_network(program::AbstractRuleNode, grammar::AbstractGrammar)::ReactionNetwork
    reactions::Vector{Reaction} = interpret_network_reactions(program.children[1], grammar)
    return ReactionNetwork(reactions)
end

function interpret_network_reactions(program::AbstractRuleNode, grammar::AbstractGrammar)::Vector{Reaction}
    ...
    @match rule begin
        :(vcat(reaction, reaction_list)) => begin
            ...
            reaction_list::Vector{Reaction} = interpret_network_reactions(program.children[2], grammar)
