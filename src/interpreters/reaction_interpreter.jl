...
function interpret_molecule_list(program::AbstractRuleNode, grammar::AbstractGrammar)::Vector{Molecule}
    rule = grammar.rules[get_rule(program)]
    ...
    @match rule begin
        ...
        :(vcat([molecule], molecule_list)) => begin
            ...
            molecule_list::Vector{Molecule} = interpret_molecule_list(program.children[2], grammar)
